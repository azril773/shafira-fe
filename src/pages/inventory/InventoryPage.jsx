...
const returnActions = [
  { id: 'abort', label: 'Abort', icon: XCircle, description: 'Batalkan proses retur' },
  { id: 'void', label: 'Void', icon: Slash, description: 'Batalkan transaksi retur' },
  { id: 'retur', label: 'Retur Barang', icon: RotateCcw, description: 'Kembalikan barang ke vendor' },
]
...
const DEMO_PURCHASE_ORDERS = [
  { id: 'PO-001', vendor: 'CV. Mitra Utama', date: '2026-04-08T08:00:00', total: 720000, status: 'Diterima', product: 'Beras 5kg', qty: 10 },
  { id: 'PO-002', vendor: 'UD. Sumber Makmur', date: '2026-04-09T09:20:00', total: 540000, status: 'Proses', product: 'Minyak Goreng 2L', qty: 12 },
  { id: 'PO-003', vendor: 'PT. Graha Logistik', date: '2026-04-10T10:18:00', total: 910000, status: 'Dibatalkan', product: 'Gula 1kg', qty: 25 },
]

const DEMO_TRANSACTIONS = [
  { id: 'TRX-001', date: '2026-04-08T08:12:00', items: 5, total: 42500, method: 'Tunai', status: 'Selesai', details: [{ id: 1, name: 'Indomie Goreng', qty: 2, price: 3500 }, { id: 2, name: 'Aqua 600ml', qty: 1, price: 4000 }, { id: 9, name: 'Beng-Beng', qty: 2, price: 3000 }] },
  { id: 'TRX-002', date: '2026-04-08T08:47:00', items: 3, total: 97000, method: 'QRIS', status: 'Selesai', details: [{ id: 4, name: 'Beras 5kg', qty: 1, price: 75000 }, { id: 5, name: 'Pocari Sweat 600ml', qty: 1, price: 8500 }, { id: 6, name: 'Sprite 390ml', qty: 1, price: 7000 }] },
  { id: 'TRX-003', date: '2026-04-08T12:45:00', items: 1, total: 3500, method: 'Tunai', status: 'Void', details: [{ id: 1, name: 'Indomie Goreng', qty: 1, price: 3500 }] },
  { id: 'TRX-004', date: '2026-04-08T14:19:00', items: 3, total: 80000, method: 'Tunai', status: 'Abort', details: [{ id: 2, name: 'Aqua 600ml', qty: 2, price: 4000 }, { id: 14, name: 'Shampoo Pantene 90ml', qty: 1, price: 12000 }] },
]

const DEMO_VENDORS = ['CV. Mitra Utama', 'UD. Sumber Makmur', 'PT. Graha Logistik']
...
export default function InventoryPage() {
  ...
  const [newPurchase, setNewPurchase] = useState({ productId: DEMO_PRODUCTS[0].id, vendor: DEMO_VENDORS[0], qty: 1, purchaseDate: new Date().toISOString().slice(0, 16) })
  const [purchaseMessage, setPurchaseMessage] = useState('')
  const [newProduct, setNewProduct] = useState({ sku: '', barcode: '', name: '', category: 'Sembako', price: '', stock: '' })
  const [productMessage, setProductMessage] = useState('')

  const salesTransactions = transactions.filter((trx) => trx.status === 'Selesai')
  const returnTransactions = transactions.filter((trx) => trx.status === 'Retur')
  const totalSales = salesTransactions.reduce((sum, trx) => sum + trx.total, 0)
  const totalPurchase = purchaseOrders.reduce((sum, order) => sum + order.total, 0)
  ...
  const topSeller = Object.entries(bestSeller).sort((a, b) => b[1] - a[1])[0]?.[0] ?? 'Belum ada data'
  const today = new Date().toDateString()
  const todayReturns = returnTransactions.filter((trx) => new Date(trx.date).toDateString() === today).length
  ...
  const reportDetails = {
    ...
    purchase: {
      ...
      metrics: [
        ...
        { label: 'Vendor Utama', value: purchaseOrders[0]?.vendor ?? 'Belum ada vendor' },
    ...
    stock: {
      ...
      metrics: [
        ...
        { label: 'Reorder Prioritas', value: lowStockItems.slice(0, 3).map((item) => item.name).join(', ') || 'Aman' },
      ],
    },
    return: {
      label: 'Retur',
  ...
  const handleExportReport = (type) => {
    ...
    if (type === 'excel') {
      exportToExcel(`${selectedReport.label.replace(/\s/g, '_')}_Report`, headers, rows)
    } else {
  ...
  const handleItemClick = (item) => {
    ...
    setInfoMessage(`Demo produk: ${item.name}. Gunakan tombol restock atau retur untuk melihat simulasi.`)
  }

  const handleInventoryAction = (action) => {
    if (!selectedItem) {
      setInfoMessage('Pilih produk terlebih dahulu untuk menjalankan demo aksi inventory.')
      return
    }
  ...
  const handleReturnAction = (actionId) => {
    ...
    setInfoMessage(`Simulasi aksi ${actionId.toUpperCase()} pada laporan retur dijalankan.`)
  }

  const handleCreatePurchase = () => {
    const product = products.find((item) => item.id === Number(newPurchase.productId))
    if (!product) {
  ...
  const handleCreateProduct = () => {
    if (!newProduct.sku || !newProduct.name || !newProduct.price || !newProduct.stock) {
      setProductMessage('Lengkapi SKU, nama, harga, dan stok produk.')
      return
    }
    ...
    setProductMessage(`Produk ${newProduct.name} berhasil ditambahkan.`)
    setNewProduct({ sku: '', barcode: '', name: '', category: 'Sembako', price: '', stock: '' })
  }
  ...
  const handleUpdatePurchaseStatus = (orderId, targetStatus) => {
        ...
        if (targetStatus === 'Diterima' && product) {
          setProducts((currentProducts) =>
            currentProducts.map((item) =>
              item.id === product.id ? { ...item, stock: item.stock + order.qty } : item,
            ),
          )
        }
        if (order.status === 'Diterima' && targetStatus === 'Dibatalkan' && product) {
          setProducts((currentProducts) =>
            currentProducts.map((item) =>
              item.id === product.id ? { ...item, stock: Math.max(0, item.stock - order.qty) } : item,
            ),
          )
        }
    ...
    setPurchaseMessage(`Status order ${orderId} diubah menjadi ${targetStatus}.`)
  }
      ...
      <main className="relative z-10 mx-auto max-w-[1440px] px-6 pb-12 pt-10">
        <div className="rounded-[40px] border border-white/60 bg-white/90 p-8 shadow-xl shadow-orange-200/20 backdrop-blur-sm">
          <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
            <div className="space-y-6">
              <p className="text-sm uppercase tracking-[0.3em] text-orange-500">Inventory</p>
              <div>
                <h1 className="text-4xl font-bold text-gray-900">Shafira Mart Inventory</h1>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-[32px] bg-orange-50 p-5 shadow-sm">
                  <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Master Item</p>
                  <p className="mt-3 text-3xl font-semibold text-gray-900">{products.length}</p>
                  <p className="mt-2 text-sm text-gray-600">Produk tersedia di katalog</p>
                </div>
                <div className="rounded-[32px] bg-orange-50 p-5 shadow-sm">
                  <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Pembelian</p>
                  <p className="mt-3 text-3xl font-semibold text-gray-900">{purchaseOrders.length}</p>
                  <p className="mt-2 text-sm text-gray-600">Order dari vendor dan status pembelian</p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8">
          {activeModule === 'master' && (
            <section className="rounded-[40px] border border-orange-100 bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Master Item</p>
                  <h3 className="mt-2 text-xl font-semibold text-gray-900">Daftar Produk</h3>
                </div>
                <button
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
                ...
                <div className="space-y-4">
                  <div className="rounded-[32px] border border-orange-100 bg-orange-50 p-5">
                    <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Demo Inventory</p>
                  ...
                  <div className="rounded-[32px] border border-orange-100 bg-white p-5 shadow-sm">
                    <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Detail Produk</p>
                    {selectedItem ? (
                      <div className="mt-4 space-y-3 text-sm text-gray-700">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          <div className="rounded-3xl bg-orange-50 p-3">
                            <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Stok</p>
                            <p className="mt-2 font-semibold text-gray-900">{selectedItem.stock}</p>
                          </div>
                          <div className="rounded-3xl bg-orange-50 p-3">
                            <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Harga</p>
                            <p className="mt-2 font-semibold text-gray-900">{formatRupiah(selectedItem.price)}</p>
                          </div>
                        </div>
                        ...
                        {selectedItemAction && (
                          <p className="mt-2 text-sm text-green-700">{selectedItemAction}</p>
                        )}
                      </div>
                    ) : (
                      <p className="mt-4 text-sm text-gray-500">Pilih produk untuk melihat demo detail dan aksi inventory.</p>
                    )}
                  </div>

                  <div className="rounded-[32px] border border-orange-100 bg-white p-5 shadow-sm">
                    <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Tambah Produk</p>
          ...
          {activeModule === 'purchases' && (
            <section className="rounded-[40px] border border-orange-100 bg-white p-6 shadow-sm">
              <div>
                <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Pembelian</p>
                <h3 className="mt-2 text-xl font-semibold text-gray-900">Beli ke Vendor</h3>
              </div>
              <div className="mt-6 grid gap-4 lg:grid-cols-3">
                <div className="rounded-3xl border border-orange-100 bg-orange-50 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Penjualan POS</p>
                  <p className="mt-3 text-3xl font-semibold text-gray-900">{formatRupiah(totalSales)}</p>
                  <p className="mt-2 text-sm text-gray-600">Total pendapatan dari demo penjualan POS.</p>
                </div>
                <div className="rounded-3xl border border-orange-100 bg-orange-50 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Total Purchase</p>
                  <p className="mt-3 text-3xl font-semibold text-gray-900">{purchaseOrders.length}</p>
                  <p className="mt-2 text-sm text-gray-600">Order pembelian vendor yang tersedia.</p>
                </div>
                <div className="rounded-3xl border border-orange-100 bg-orange-50 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-orange-500">Stok Rendah</p>
                  <p className="mt-3 text-3xl font-semibold text-gray-900">{lowStockItems.length}</p>
                  <p className="mt-2 text-sm text-gray-600">Produk yang perlu restock berdasarkan demo stok.</p>
                </div>
              </div>
              <div className="mt-6 rounded-[32px] border border-orange-100 bg-orange-50 p-5">
                <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Form Demo Pembelian</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  ...
                  <label className="block text-sm text-gray-600">
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPurchase({ ...newPurchase, qty: Number(e.target.value) })}
                      className="mt-2 w-full rounded-xl border border-orange-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300"
                    />
                  </label>
                  <label className="block text-sm text-gray-600">
              ...
              <div className="mt-6 space-y-4">
                {purchaseOrders.map((order) => (
                  <div key={order.id} className="rounded-3xl border border-orange-100 bg-orange-50 p-5">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <p className="text-sm text-gray-500">Vendor</p>
                        <p className="mt-1 font-semibold text-gray-900">{order.vendor}</p>
                        <p className="mt-2 text-xs text-gray-500">Produk: {order.product}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-500">Total</p>
                        <p className="mt-1 font-semibold text-gray-900">{formatRupiah(order.total)}</p>
                      </div>
                    </div>
                    <p className="mt-3 text-sm text-gray-600">Status: <span className="font-semibold">{order.status}</span></p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {order.status === 'Proses' && (
                        <>
                          <button
                            type="button"
                            onClick={() => handleUpdatePurchaseStatus(order.id, 'Diterima')}
                          ...
                          <button
                            type="button"
                            onClick={() => handleUpdatePurchaseStatus(order.id, 'Dibatalkan')}
            ...
            <section className="rounded-[40px] border border-orange-100 bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-xl font-semibold text-gray-900">Laporan Inventaris</h3>
                  <p className="mt-2 text-sm text-gray-600">Akses cepat ke report penjualan, pembelian, stok, dan retur barang.</p>
                </div>
                ...
                {reportItems.map((report) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-gray-600">
                      {report.id === 'sales' && 'Lihat ringkasan penjualan produk.'}
                      {report.id === 'purchase' && 'Pantau pembelian dan vendor.'}
                      {report.id === 'stock' && 'Cek stok barang di gudang.'}
                      {report.id === 'return' && 'Kelola retur dengan opsi abort, void, atau retur barang.'}
                    </p>
                  </button>
                ))}
              </div>
              <div className="mt-6 rounded-[32px] border border-orange-100 bg-orange-50 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-orange-500 uppercase tracking-[0.2em]">Detail Report</p>
                    <h3 className="mt-2 text-xl font-semibold text-gray-900">{selectedReport.label}</h3>
                  </div>
                  <span className="rounded-full bg-white/80 px-4 py-2 text-sm font-semibold text-orange-700 border border-orange-100">
                    Rekomendasi
                  </span>
                </div>
                <p className="mt-4 text-sm text-gray-600">{selectedReport.summary}</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {selectedReport.metrics.map((metric) => (
                    <div key={metric.label} className="rounded-3xl border border-orange-100 bg-white p-4 shadow-sm">
                      <p className="text-xs uppercase tracking-[0.2em] text-orange-500">{metric.label}</p>
              ...
              <div className="mt-6 rounded-[32px] border border-orange-100 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Preview {selectedReport.label}</p>
                    <h4 className="mt-2 text-lg font-semibold text-gray-900">Data contoh di web</h4>
                  </div>
                  <span className="text-sm font-semibold text-gray-600">Menampilkan {reportPreviewRows.length} baris</span>
                </div>
                  ...
                  <table className="min-w-full divide-y divide-orange-100 text-left text-sm text-gray-600">
                    ...
                    <tbody className="divide-y divide-orange-100 bg-white">
                      {reportPreviewRows.slice(0, 5).map((row, index) => (
                        <tr key={`${row.ID ?? index}-${index}`} className="hover:bg-orange-50">
                          {exportHeaders[activeReport].map((header) => (
                      ...
                      {reportPreviewRows.length === 0 && (
                        <tr>
                          <td colSpan={exportHeaders[activeReport].length} className="px-4 py-5 text-center text-sm text-gray-500">
                ...
                {reportPreviewRows.length > 5 && (
                  <p className="mt-3 text-xs text-gray-500">Preview dibatasi 5 baris. Gunakan export untuk melihat seluruh data.</p>
                )}
              </div>
              {activeReport === 'return' && (
                <div className="mt-6">
                  ...
                  {activeReturnAction && (
                    <div className="mt-4 rounded-3xl border border-orange-100 bg-orange-50 p-4 text-sm text-orange-700">
                      Aksi retur dipilih: <span className="font-semibold">{activeReturnAction}</span>. Lihat rekomendasi dan status retur di atas.
                    </div>
                  )}
                </div>
              )}
